...
def append_row(data, sC, sD, n):
    data_new = []
    data_new.append([''] + [str(i) for i in range(sC + sD)])
    for i in range(n):
        data_new.append(['G' + str(i)] + [str(val) for val in data[i]])
    return data_new


def convert_to_integer(L):
    M = [[0 for i in range(len(L[0]) - 1)] for j in range(len(L) - 1)]
...
def normalize(E):
    ...
    # Step 1. Row-wise geometric mean
    GM = []
    for i in range(row):
        GM.append(geo_mean_overflow(E[i, :]))
    # print (GM)

    # Step 2. Divide each gene by its row-wise geometric mean
    E = np.array([E[i, :]/GM[i] for i in range(row)])
    # print (E)

    # Step 3. Calculate size ratio of each sample by calculating median of each sample (column)
    SR = np.median(E, axis = 0)
    # print (SR)

    # Step 4. Calculate size ratio by calculating median of each sample (column)
    E = np.transpose([E[:, i]/SR[i] for i in range(col)])
    # print (E)
    return E
...
def GENIE3(expr_data, nthreads, gene_names=None, regulators='all', tree_method = 'RF', K = 'sqrt', ntrees = 1000):
